...
def test_feature(
    cursor: psycopg2._psycopg.cursor, feature_category: str, feature_name: str
) -> Literal["full"] | Literal["no"] | Literal["partial"]:
    support = "no"
    try:
        ...
        if feature_category == "data_types":
            ...
            if feature_name == "Schemas":
                ...
                cursor.execute(
                    "CREATE MATERIALIZED VIEW test_matview AS SELECT 1 AS col;"
                )
            ...
            if feature_name == "Extension Support":
                ...
                extensions = cursor.fetchone()
                if extensions is None or extensions[0] == 0:
                    support = "no"
                else:
                    support = "full"
                return support
            ...
            if feature_name == "Index Types":
                ...
                support = "partial"
                cursor.execute(
                    "CREATE INDEX test_gist ON test_fts USING gist (content);"
                )
                support = "partial"
                cursor.execute(
                ...
                if (
                    "test_part2" in partitions_in_plan
                    and "test_part1" not in partitions_in_plan
                ):
                    return "full"
                else:
                    raise Exception(
                        "Partition Pruning test failed: Incorrect partitions included in the plan."
                    )
                ...
                if result is None:
                    return "no"
                wal_lsn_before = result[0]
                cursor.execute(
                    "insert into unlogged_pci_demo select generate_series, 'N',lpad('x',generate_series,'x') from generate_series(1,10000);"
                )
                cursor.execute(
                    f"select (pg_wal_lsn_diff(pg_current_wal_lsn(),'{wal_lsn_before}')) from pg_stat_database where datname=current_database();"
                )
                result = cursor.fetchone()
                if result is None:
                    return "no"
                ...
                if diff_after < 50000:
                    ...
                    raise Exception(
                        "Unlogged Table test failed: Excessive WAL Generated."
                    )
            ...
            if feature_name == "Logical Replication":
                cursor.execute(
                    "CREATE TABLE test_replication (id INT PRIMARY KEY, value TEXT);"
                )
                cursor.execute(
                    "CREATE PUBLICATION test_pub FOR TABLE test_replication WHERE (id > 10 and value <>'UNKNOWN');"
                )
                cursor.execute(
                ...
                if result is None or result[0] != "test_pub":
                    raise Exception(
                        "Logical Replication Publication not found or misconfigured."
                    )
                else:
...
def main():
    ...
    # Run tests
    pci_results: dict[str, dict[str, str]] = {
        category: {} for category in FEATURES.keys()
    }
    for category, subfeatures in FEATURES.items():
        for subfeature in subfeatures:
            pci_results[category][subfeature] = test_feature(
                cursor, category, subfeature
            )
